...
function scanAerialsModern(root: string, fs: Fs): WallpaperEntry[] {
  ...
  for (const name of names) {
    ...
    entries.push(aerialEntry(id, titles.get(id) ?? id, joinPath(videosDir, name), thumbnail, fs))
  }
  return entries
}
...
function scanAerialsLegacy(root: string, fs: Fs): WallpaperEntry[] {
  ...
  for (const name of names) {
    ...
    for (const video of videos) {
      ...
      entries.push(aerialEntry(id, titles.get(id) ?? id, joinPath(sub, video), null, fs))
    }
  }
  return entries
}
