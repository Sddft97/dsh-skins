...
describe('skin controller', () => {
  ...
  it('disposes previous activation before new hooks execute so old cleanup does not clobber new state', async () => {
    let hookExecutionSkinId: string | null = null
    const hookStateHistory: string[] = []
    const ledger = createEffectLedger()
    ...
    const createHookModule = (name: string) => ({
      default: () => ({
        apply: (ctx: { onCleanup: (fn: () => void) => void }) => {
          hookExecutionSkinId = document.documentElement.getAttribute('data-dsh-skin')
          document.body.setAttribute(`data-${name}-active`, 'true')
          hookStateHistory.push(`${name}:apply`)
          ctx.onCleanup(() => {
            document.body.removeAttribute(`data-${name}-active`)
            hookStateHistory.push(`${name}:cleanup`)
          })
        },
      }),
    })
    const controller = createSkinController({
      ...
      ledger,
      loadStylesheet,
      importHooks: async (path) => {
        if (path.includes('skin-a')) return createHookModule('skin-a')
        if (path.includes('skin-b')) return createHookModule('skin-b')
        return undefined
      },
      persist: async () => {},
    })

    const hookedEntryFor = (id: string): ControllerSkinEntry => ({
      manifest: {
        id,
        contributes: { stylesheet: 'skin.css' },
        facets: { client: { entry: 'hooks.mjs', apiVersion: 'x-org.linxin666.skin-center/v1alpha1' } },
    ...
    // 1. Switch to skin-a
    await controller.switchTo('skin-a', hookedEntryFor('skin-a'))
    expect(hookExecutionSkinId).toBe('skin-a')
    expect(document.body.hasAttribute('data-skin-a-active')).toBe(true)
    expect(hookStateHistory).toEqual(['skin-a:apply'])

    // 2. Switch to skin-b: skin-a's cleanup must run BEFORE skin-b's apply
    await controller.switchTo('skin-b', hookedEntryFor('skin-b'))
    expect(hookExecutionSkinId).toBe('skin-b')
    expect(document.body.hasAttribute('data-skin-a-active')).toBe(false)
    expect(document.body.hasAttribute('data-skin-b-active')).toBe(true)
    expect(hookStateHistory).toEqual(['skin-a:apply', 'skin-a:cleanup', 'skin-b:apply'])

    // 3. Switch back to skin-a (try-on then switch, or repeat)
    await controller.switchTo('skin-a', hookedEntryFor('skin-a'))
    expect(hookExecutionSkinId).toBe('skin-a')
    expect(document.body.hasAttribute('data-skin-a-active')).toBe(true)
    expect(document.body.hasAttribute('data-skin-b-active')).toBe(false)
    expect(hookStateHistory).toEqual(['skin-a:apply', 'skin-a:cleanup', 'skin-b:apply', 'skin-b:cleanup', 'skin-a:apply'])
  })
