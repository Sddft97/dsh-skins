...
const GENERATED = join(ROOT, 'src', 'reviewed-hooks.generated.ts')
...
test('check mode reports a stale generated registry without rewriting it', () => {
  ...
  const copiedGenerated = join(copyRoot, 'src', 'reviewed-hooks.generated.ts')
  try {
    ...
    const sourceSkins = join(ROOT, 'skins')
    const targetSkins = join(copyRoot, 'skins')
